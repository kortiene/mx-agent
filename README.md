...
| `workspace`, `agent`, `approval`, `share`, `invocation` commands (and `trust publish`/`state`) fully daemon-IPC-mediated — the CLI never restores a Matrix session/client. **`auth login` is a CLI-initiated, same-binary exception**: it builds a store-backed Matrix client and creates the daemon-owned crypto store in-process; `auth status`/`logout` and `trust list`/`approve`/`revoke`/`fingerprint` run CLI-local against `session.json`/`trust.json`/`signing_key.ed25519` (see [architecture §10.3](docs/architecture.md)) | ✅ Implemented |
...
| Live daemon scheduler loop: auto-claims assigned, signed, policy-allowed tasks from room state and runs them, with restart recovery | ✅ Implemented (the daemon signs CLI-authored `tool`/`exec` actions on the local IPC caller's behalf — the CLI never holds the signing key; local tool/exec dispatch by default; signed Matrix-backed `call`/`exec` task dispatch is opt-in via `MX_AGENT_TASK_DISPATCH=matrix`) |
| `call` / `exec` runners | 🟡 `call` and `exec` (batch and interactive `--pty`) support signed Matrix-backed remote daemon dispatch when `--room`/`--agent` are provided; live remote exec supports signed stdin/cancel controls and PTY resize; a `requires_approval` live `exec`/`call` is held fail-closed and then **released** on a verified, signed, unexpired `approval approve` decision (re-running the full authorize pipeline before spawning), **denied** on `approval deny`, or **expired** fail-closed past its window — none ever run without a verified decision (issue #306) |
| Sandbox backends | ✅ Implemented (`none` fallback by default; `bubblewrap`, `docker`, and `podman` are policy-selectable — `firejail`/`chroot` are rejected at policy load, never silently unsandboxed; the runtime follows the `sandbox` value and the image is `execution.container_image`; `read_only_paths`/`writable_paths` bind-mount confinement and `network` policy enforced end-to-end for batch exec **and** interactive `--pty`; bubblewrap hardens with a user namespace, `--cap-drop ALL`, private `/proc`+`/dev`+tmpfs, and `--new-session` (batch); containers run as the daemon's identity (`--user`/`--userns=keep-id`) with `--cap-drop ALL`, add `no-new-privileges`, and pass env by name (values never in argv); policy-driven resource caps (`max_processes`/`max_memory_bytes`/`max_cpu_seconds`) via container cgroup flags or a `setrlimit` re-exec launcher on the host paths; `execution.require_sandbox` denies executions resolving to `none`; seccomp ships a curated default-deny BPF profile (`ERRNO(EPERM)`, opt-in, default off, Linux) installed on every backend — in-process on `none`, `bwrap --seccomp` on bubblewrap, `--security-opt seccomp=` on containers; a missing backend binary fails closed with an actionable diagnostic; per-invocation output byte cap on PTY (agent's `max_output_bytes`, 64 MiB loopback default)) |
| E2EE privileged-event handling | ✅ Implemented (decrypts privileged events; fails safe on undecryptable events) |
| Encryption-on-create | ✅ Implemented (opt-in): `workspace create --e2ee on` makes the room born encrypted (Megolm v1 via `initial_state`) and reports `encrypted: true`; default remains unencrypted (`--e2ee off`). Under `--e2ee on`, **timeline** events and the **media offload** (>256 KiB exec output and large shares, Matrix `EncryptedFile` scheme) are encrypted and unreadable by the homeserver operator; Matrix **state** events (task action `command`/`env`/`result`, invocation/agent/workspace state) are not Megolm-encrypted and stay plaintext readable by the operator — do not place secrets in a task action's `env` (issue #308). Encryption is a transport property only — signing+trust+policy+approval remain the execution gate. Turning E2EE on by default is a separate, not-yet-scheduled rollout |
| Multi-agent workspace power levels | ✅ Implemented: `workspace create` provisions per-event-type power levels (each `com.mxagent.*` state type requires PL 50, `state_default` 100, joiners PL 0), and the creator elevates a joining daemon with `workspace grant --room <id> --user <@mxid>`. A refused state write surfaces a guided `M_FORBIDDEN` instead of a raw 403. Power levels gate state-write integrity/DoS only — never execution, which stays with signing+trust+policy+approval (issue #301) |
...
| Large-output artifact mode | ✅ Implemented (Matrix media offload, SHA-256 integrity, optional zstd compression, tail preview; in `--e2ee on` rooms the media blob is uploaded as ciphertext via the Matrix `EncryptedFile` scheme and decrypted on retrieval — issue #308); very-large-output tuning remains planned |
...
| Interactive PTY over IPC/remote | ✅ Implemented (daemon-allocated PTY streamed over IPC; remote `--room`/`--agent` PTY over the signed Matrix transport with stdin/resize/cancel; merged PTY output is byte-capped and reports `truncated` on completion) |
...
The daemon owns all long-lived state at rest (Matrix session, keys, policy). The CLI is stateless and talks to it over `$XDG_RUNTIME_DIR/mx-agent/daemon.sock`. The `workspace` / `agent` / `approval` / `share` / `invocation` and `task` command groups (and `trust publish`/`state`) run against a real Matrix homeserver entirely through the daemon over local IPC today, so for those the CLI never reads the Matrix session file or builds a Matrix client. **`auth login` is a deliberate CLI-initiated exception** (issue #201): it receives the password, builds a store-backed Matrix client, performs the network login, and creates the daemon-owned crypto store **in-process**, then hands the session to the daemon-owned data dir. `auth status`/`logout` and the local `trust list`/`approve`/`revoke`/`fingerprint` commands likewise read/write `session.json`/`trust.json`/`signing_key.ed25519` directly. This is safe only because the CLI and daemon are the **same binary at the same UID**; an advisory file lock serializes those in-process writes against a running daemon (see [architecture §10.3](docs/architecture.md#103-ipc-protocol)). `call` and `exec` — including interactive `exec --pty` — are daemon-mediated local loopback by default and become signed Matrix-backed remote operations when `--room` and `--agent` target a registered, trusted, policy-allowed remote agent. Live remote exec stdin/cancel controls are signed and policy/ownership checked by the target daemon; an interactive PTY's window-resize is likewise a signed control event the target verifies by signature, trust, and requester ownership before applying. See [Project status](#project-status) for the full breakdown.
...
- Every privileged request is **Ed25519-signed** and checked against **local policy** before running anything; request types that carry nonce/expiry fields are also replay/expiry checked. Replay protection **fails closed** — if the replay cache cannot be loaded or is corrupt, the daemon skips the work (routes nothing / skips the scheduler pass) rather than running unchecked — and live control frames (`exec.stdin`/`exec.cancel`/`pty.resize`) are replay-checked **per live session**.
