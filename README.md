...
| Sandbox backends | ✅ Implemented (`none` fallback by default; `bubblewrap` and Docker/Podman container backends are policy-selectable; `read_only_paths`/`writable_paths` bind-mount confinement and `network` policy enforced end-to-end for batch exec; interactive `--pty` has baseline controls plus a per-invocation output byte cap (the agent's `max_output_bytes` for live PTY, a 64 MiB default for loopback), and the container backend allocates an in-container TTY (`-i -t`) so `isatty` is true inside the container; no seccomp/rlimit/UID-GID remap) |
