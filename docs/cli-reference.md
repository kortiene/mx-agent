...
When `--room` and `--agent` are omitted, the command runs in the daemon's local process supervisor (loopback). The daemon sends all stdout/stderr as a sequence of `StreamChunk` frames (utf-8 or base64 encoded) followed by an `ExecFinished` frame carrying the exit status. The CLI renders these frames to its own stdout/stderr in real time. Loopback execution applies the operator's policy **confinement floor** (`Policy::execution_allowance()` — sandbox backend, network decision, filesystem binds, and env allowlist), the same floor a built-in `call` gets; with no `policy.toml` the fail-closed defaults apply. It also enforces a **default timeout** (600 s) and a **default output cap** (64 MiB): a command exceeding the cap reports `truncated: true` in `ExecFinished`, and a command exceeding the timeout is terminated (its whole process group is signalled).
...
seccomp = "off"                       # off (default) | default — installs a default-deny syscall-filter profile (Linux)
...
| `execution.seccomp` | workspace | enum | `off` (default) or `default`; `default` installs a curated default-deny (`ERRNO(EPERM)`) BPF profile on every Linux backend, opt-in (issues #349, #380) |
...
| `rooms."ROOM_ID".agents."AGENT_ID".seccomp` | agent | enum | Overrides `execution.seccomp` (`off`/`default`); `default` installs the default-deny profile (Linux) (issues #349, #380) |
