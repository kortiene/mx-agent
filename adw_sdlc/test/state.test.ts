...
describe('persistence', () => {
  ...
  it('serializes additive fields when set', () => {
    const state = new AdwState({ adwId: 'a1b2c3d4', engine: 'ts', runner: 'claude', totalCostUsd: 0.5 });
    state.save();
 ...
 * uses (type/required/properties/items/pattern/minimum) — the TS mirror of
 * adw/test_state.py _validate, the other half of the dual-language check.
 */
function validate(instance: unknown, schema: Schema, path = '$'): string[] {
  const errors: string[] = [];
