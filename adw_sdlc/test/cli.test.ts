...
describe('main — engine dispatch', () => {
  it('delegates to the py engine by default, forwarding argv verbatim', async () => {
    ...
    // the passthru are forwarded untouched and the py engine applies its own
    // validation (pi|claude) exactly as a direct invocation would.
    const rc = await main(['5', '--runner', 'gemini', '--yes', '--', '--model', 'x'], deps);
    expect(rc).toBe(3);
