...
/**
 * Default test gate. `testCmd` overrides only this (the test step); the
 * other pre-merge gates below always run.
 */
export const DEFAULT_TEST_CMD = 'cargo test --all';
...
/**
 * Pre-merge gate commands. `testCmd` overrides only the *test* gate (the
 * first entry); fmt/clippy/build still run, so narrowing the tests never
 * silently drops the other quality gates before the irreversible squash-merge.
 */
export function finalizeGates(testCmd: string): string[] {
  const gates: string[] = [...DEFAULT_FINALIZE_GATES];
  if (testCmd) {
    gates[0] = testCmd;
  }
  return gates;
...
async function finalizeAndMerge(
  ...
  for (const gate of finalizeGates(opts.testCmd)) {
...
function printPlan(issue: number, runner: AgentRunner, phases: readonly string[], opts: ResolvedOptions): void {
  ...
  console.log(`[dry-run] test gate: ${opts.testCmd || DEFAULT_TEST_CMD}`);
}
