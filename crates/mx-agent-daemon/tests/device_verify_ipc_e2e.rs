//! End-to-end IPC tests for the `device.verify.start` timeout and concurrency
//! fix (issue #258).
//!
//! These tests exercise the critical-path behavior of the decision-timeout fix
//! at the IPC + dispatch level, **without requiring a live Matrix homeserver**.
//! They simulate the `dispatch_device_verify` handler's phase-2 behavior: the
//! handler streams `Started` and `EmojiReady` frames, then calls
//! [`read_verify_decision`] with a bounded timeout and writes a terminal frame.
//!
...
//!
//! Neither test requires Docker, a Matrix homeserver, or any external service.
//! Both run as part of the default `cargo test --all`.
