...
> **Implementation status.** The orchestration *model* (agents, signed exec/call, task DAG, approvals) is fully specified, and the protocol/policy/signing layers plus most command groups are implemented. In v0.1.0 the `auth` / `workspace` / `agent` / `trust` / `approval` / `share` and `task` command groups run against a real Matrix homeserver through the daemon; task state CRUD/graph/watch/diagnostics work today, and the daemon **task-orchestration engine** (scheduler, optimistic `state_rev` claiming, tool/exec dispatch, policy + trust/signature + approval enforcement, restart recovery) is implemented and tested. The two main gaps: `call` / `exec` still run **local-loopback** (the signed Matrix transport to a *remote* daemon is landing, #155), and the orchestration engine is **not yet auto-driven by a live `/sync` loop**, so a running daemon does not yet poll and execute tasks on its own. Commands below are tagged where behavior is not yet live.
