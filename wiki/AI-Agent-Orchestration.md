...
> **Implementation status.** The orchestration *model* (agents, signed exec/call, task DAG, approvals) is fully specified and the protocol/policy/signing layers are implemented. Most CLI subcommands that drive it are **🟡 alpha stubs in v0.1.0** — they parse arguments and return "not implemented yet" while daemon behavior lands. The single-machine **local-loopback** path makes it possible to exercise the full request→sign→policy→run pipeline today. Commands below are tagged where behavior is not yet live.
