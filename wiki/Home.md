...
> **Project status (v0.2.0, public alpha).** The architecture, protocol schema, IPC layer, policy engine, Ed25519 signing, and sandbox abstraction are in place. All command groups run against a real Matrix homeserver entirely through the daemon (the stateless CLI never holds a Matrix session). `call` and `exec` (batch and interactive `--pty`) support signed Matrix-backed remote daemon dispatch when `--room`/`--agent` are provided, including signed stdin/resize/cancel controls; `--pty` streams the daemon's pseudo-terminal over local IPC and the signed Matrix transport. A **live daemon scheduler loop** auto-claims assigned, signed, policy-allowed tasks from room state and runs them, with restart recovery. CI (`matrix-integration`) is green and stable. Throughout this wiki, each feature is tagged **✅ Implemented**, **🟡 Designed / in progress**, or **🔮 Planned** so you always know what runs today.
