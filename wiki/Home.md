...
> **Project status (v0.1.0, public alpha).** The architecture, protocol schema, IPC layer, policy engine, Ed25519 signing, and sandbox abstraction are in place. Most command groups run against a real Matrix homeserver entirely through the daemon (the stateless CLI never holds a Matrix session). `call` and non-PTY `exec` support signed Matrix-backed remote daemon dispatch when `--room`/`--agent` are provided, including signed stdin/cancel controls. A **live daemon scheduler loop** auto-claims assigned, signed, policy-allowed tasks from room state and runs them, with restart recovery. CI (`matrix-integration`) is green and stable. Throughout this wiki, each feature is tagged **✅ Implemented**, **🟡 Designed / in progress**, or **🔮 Planned** so you always know what runs today.
